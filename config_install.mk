...
uninstall: has_installer
	@if [ ! -d "${CLUSTER_PATH}/" ]; then \
	  echo "time ${OPENSHIFT_INSTALLER} destroy cluster --dir=${CLUSTER_PATH} --log-level=debug 2>&1 | tee ${CLUSTER_PATH}/uninstall.log"; \
	  echo "ERROR: Cluster not found ${CLUSTER_PATH}"; \
	  exit 1; \
	fi
